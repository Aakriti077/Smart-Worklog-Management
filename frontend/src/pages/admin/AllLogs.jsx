...
export default function AllLogs() {
  ...
  const departments = [...new Set(users.map(u => u.department_name).filter(Boolean))].sort()
  ...
  const filtered = logs.filter(l => {
    ...
    const matchDept = !deptFilter || users.find(u => u.id === l.user)?.department_name === deptFilter
    return matchSearch && matchEmp && matchDept && dateInRange(l)
  })

  useEffect(() => setPage(1), [search, empFilter, deptFilter, period, dateFilter])
      ...
      <div className="card">
        <div className="card-header" style={{ padding: '16px 24px', flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginLeft: 'auto' }}>
            <select className="form-input" style={{ width: 155, padding: '5px 10px' }} value={deptFilter} onChange={e => { setDeptFilter(e.target.value); setEmpFilter('') }}>
              <option value="">All Departments</option>
              {departments.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
            <select className="form-input" style={{ width: 150, padding: '5px 10px' }} value={empFilter} onChange={e => setEmpFilter(e.target.value)}>
              <option value="">All Employees</option>
              {(deptFilter ? users.filter(u => u.department_name === deptFilter) : users).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
            <select
        ...
        <div className="table-wrap" style={{ minHeight: 580 }}>
          <table className="table">
            ...
            <tbody>
              {paged.map(l => (
                <tr key={l.id}>
                  <td className="text-sm text-muted" style={{ whiteSpace: 'nowrap' }}>{l.date}</td>
                  <td style={{ whiteSpace: 'nowrap' }}><strong>{l.user_name}</strong></td>
                  <td style={{ maxWidth: 380, fontSize: 13, lineHeight: 1.55, wordBreak: 'break-word' }}>
                    {l.log_text}
                  </td>
